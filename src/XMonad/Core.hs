...
newtype ScreenDetail   = SD { screenRect :: Xlib.Rectangle } deriving (Eq,Show, Read)
...
recompile = io $ do
    ...
    status <- bracket (openFile err WriteMode) hClose $ \errHandle ->
        waitForProcess =<< compileScript bin cfgdir buildscript errHandle
    ...
    if status == ExitSuccess
        then trace "XMonad recompilation process exited with success!"
            ...
            ghcErr <- readFile err
            let msg = unlines $
                    ["Error detected while compiling xmonad configuration file: " ++ buildscript]
